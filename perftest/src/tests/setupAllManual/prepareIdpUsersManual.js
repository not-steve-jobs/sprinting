...
const TEST_USERS_TEMPLATE_FILE_PATH = path.join(__dirname, '../../test-users-templates.json');
...
const run = async () => {
  try {
    ...
    testUsers.tenantIds.forEach((x, i) => {
      testUsers.tenantIds[i].users = [x.users[0]];
    });
    ...
    const page = await context.newPage();
    await login(page, testUsersByTenantId[0]);
    await page.close();
