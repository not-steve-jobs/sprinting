import {lhhSwi} from '../data/tenant.data';
...
const disableReasons: FeatureConfiguration = getDefaultDisableReasonsFeatureConfiguration(lhhSwi.id);
const myColleagues: FeatureConfiguration = getDefaultMyColleaguesFeatureConfiguration(lhhSwi.id);
const myColleaguesForm: FeatureConfiguration = {
  ...getDefaultMyColleaguesFormFeatureConfiguration(lhhSwi.id),
  config: {
    sections: [
      {
...
const staffingRequests: FeatureConfiguration = getDefaultStaffingRequestsFeatureConfiguration(lhhSwi.id);
const invitation: FeatureConfiguration = getDefaultInvitationFeatureConfiguration(lhhSwi.id);
const orderDetailsTimeline: FeatureConfiguration = getDefaultOrderDetailsTimelineConfiguration(lhhSwi.id);
const orderDetailsListingStatuses: FeatureConfiguration = getDefaultOrderDetailsListingStatusesConfiguration(lhhSwi.id);
const supportedFileTypesForUpload: FeatureConfiguration = getDefaultSupportedFileTypesForUploadFeatureConfiguration(
  lhhSwi.id,
);
const candidatesList: FeatureConfiguration = getDefaultCandidatesListFeatureConfiguration(lhhSwi.id);
const defaultCasesFeatureConfiguration: FeatureConfiguration = getDefaultCasesFeatureConfiguration(lhhSwi.id);
...
const emptyOrderDetailsActions: FeatureConfiguration = getEmptyOrderDetailsActionsFeatureConfiguration(lhhSwi.id);
const location: FeatureConfiguration = getDefaultLocationFeatureConfiguration(lhhSwi.id);
const contracts: FeatureConfiguration = getDefaultContractsFeatureConfiguration(lhhSwi.id);
const invoices: FeatureConfiguration = getDefaultInvoicesFeatureConfiguration(lhhSwi.id);
const mailProvider: FeatureConfiguration = getDefaultMailProviderFeatureConfiguration(lhhSwi.id);
const jobRoleTemplate: FeatureConfiguration = getDefaultJobRoleTemplateFeatureConfiguration(lhhSwi.id);
const mainMenu: FeatureConfiguration = {
  ...getDefaultMainMenuFeatureConfiguration(lhhSwi.id),
  config: {
    menuItems: [
...
const localization: FeatureConfiguration = {
  ...getDefaultLocalizationFeatureConfiguration(lhhSwi.id),
  config: {
    ...defaultLocalizationFeatureConfiguration,
    languages: [
...
const communications: FeatureConfiguration = {
  ...getDefaultCommunicationsFeatureConfiguration(lhhSwi.id),
  config: {
    ...defaultCommunicationsFeatureConfiguration,
    webKeys: [
...
const createJobOrderType: FeatureConfiguration = {
  ...getDefaultCreateJobOrderTypeConfiguration(lhhSwi.id),
  config: {
    availableTypes: [36, 37],
    rateType: 101,
  },
};
const createJobOrderFormTemporary: FeatureConfiguration = {
  ...getDefaultCreateJobOrderFormTemporaryConfiguration(lhhSwi.id),
  config: {
...
const createJobOrderFormPermanent: FeatureConfiguration = {
  ...getDefaultCreateJobOrderFormPermanentConfiguration(lhhSwi.id),
  config: {
    initialStep: 0, // steps starts from '0'
    formSteps: [
      // step 1
...
/**
 * Feature Configuration for Adecco LHH Switzerland
 *
 * ID: 221
 * Brand: LHH
 * Country: CH
 */
export const featureConfigurationDataTenantlhhSwi: FeatureConfiguration[] = [
  disableReasons,
  localization,
  location,
  contracts,
